...
class MatchEnergy() extends RecipeLogging {
  ...
  private final val KJ_13_REGEX = """<th scope="row">Energy kJ</th><td>([0-9.]+)kJ</td>""".r.unanchored
  // Matching: <th scope="row">-</th><td>970 kJ</td>
  private final val KJ_14_REGEX = """<th scope="row">-</th><td>([0-9.]+) kJ</td>""".r.unanchored
  // Matching: <th scope="row">Energy</th><td>105kJ/</td>
  private final val KJ_15_REGEX = """<th scope="row">Energy</th><td>([0-9.]+)kJ/</td>""".r.unanchored
  // Matching: <th scope="row">-</th><td>1588kJ</td>
  private final val KJ_16_REGEX = """<th scope="row">-</th><td>([0-9.]+)kJ</td>""".r.unanchored
  // Matching: <th scope="row">Energy Value kJ</th><td>Trace</td>
  private final val KJ_17_TRACE_REGEX = """<th scope="row">Energy Value kJ</th><td>(Trace)</td>""".r.unanchored
  ...
  private final val KCAL_06_REGEX = """<th scope="row">Energy \(Kcal\)</th><td>([0-9.]+)</td>""".r.unanchored
  // Matching: <th scope="row">Energy kcal</th><td>58kcal</td>
  private final val KCAL_07_REGEX = """<th scope="row">Energy kcal</th><td>([0-9.]+)kcal</td>""".r.unanchored
  // Matching: <th scope="row">Energy</th><td>233 kcal</td>
  private final val KCAL_08_REGEX = """<th scope="row">Energy</th><td>([0-9.]+) kcal</td>""".r.unanchored
  ...
  private final val KJ_KCAL_19_REGEX = """<th scope="row">Energy</th><td>([0-9.]+)/([0-9.]+)</td>""".r.unanchored
  // Matching: <th scope="row">Energy kJ/kcal</th><td>349/82</td>
  private final val KJ_KCAL_20_REGEX = """<th scope="row">Energy kJ/kcal</th><td>([0-9.]+)/([0-9.]+)</td>""".r.unanchored
  // Matching: <th scope="row">Energy</th><td>239KJ/57Kcal</td>
  private final val KJ_KCAL_21_REGEX = """<th scope="row">Energy</th><td>([0-9.]+)KJ/([0-9.]+)Kcal</td>""".r.unanchored
  // Matching: <th scope="row">Energy kJ/kcal</th><td>93kJ/22kcal</td>
  private final val KJ_KCAL_22_REGEX = """<th scope="row">Energy kJ/kcal</th><td>([0-9.]+)kJ/([0-9.]+)kcal</td>""".r.unanchored
  // Matching: <th scope="row">Energy:</th><td>325 kcal - 1360 kJ</td>
  private final val KJ_KCAL_23_REGEX = """<th scope="row">Energy:</th><td>([0-9.]+) kcal - ([0-9.]+) kJ</td>""".r.unanchored
  // Matching: <th scope="row">Energy, kJ/kcal</th><td>423 / 100</td>
  private final val KJ_KCAL_24_REGEX = """<th scope="row">Energy, kJ/kcal</th><td>([0-9.]+) / ([0-9.]+)</td>""".r.unanchored
  // Matching: <th scope="row">Energy (kJ/kcal)</th><td>234/ 55</td>
  private final val KJ_KCAL_25_REGEX = """<th scope="row">Energy \(kJ/kcal\)</th><td>([0-9.]+)/ ([0-9.]+)</td>""".r.unanchored
  // Matching: <th scope="row">Energy</th><td>160KJ40Kcal</td>
  private final val KJ_KCAL_26_REGEX = """<th scope="row">Energy</th><td>([0-9.]+)KJ([0-9.]+)Kcal</td>""".r.unanchored
  // Matching: <th scope="row">Energy (kJ/kcal)</th><td>121/28kcal</td>
  private final val KJ_KCAL_27_REGEX = """<th scope="row">Energy \(kJ/kcal\)</th><td>([0-9.]+)/([0-9.]+)kcal</td>""".r.unanchored
  // Matching: <th scope="row">Energy</th><td>239KJ/57kcal</td>
  private final val KJ_KCAL_28_REGEX = """<th scope="row">Energy</th><td>([0-9.]+)KJ/([0-9.]+)kcal</td>""".r.unanchored
  // Matching: <th scope="row">Energy (kJ/kcal)</th><td>232/ 55</td>
  private final val KJ_KCAL_29_REGEX = """<th scope="row">Energy \(kJ/kcal\)</th><td>([0-9.]+)/ ([0-9.]+)</td>""".r.unanchored
  // Matching: <th scope="row">Energ</th><td>217kJ (51kcal)</td>
  private final val KJ_KCAL_30_REGEX = """<th scope="row">Energ</th><td>([0-9.]+)kJ \(([0-9.]+)kcal\)</td>""".r.unanchored
  // Matching: <th scope="row">Energy</th><td>160KJ/40kcal</td>
  private final val KJ_KCAL_31_REGEX = """<th scope="row">Energy</th><td>([0-9.]+)KJ/([0-9.]+)kcal</td>""".r.unanchored
  // Matching: <th scope="row">Energy</th><td>0 kJ/kcal</td>
  private final val KJ_KCAL_32_SAME_REGEX = """<th scope="row">Energy</th><td>([0-9.]+) kJ/kcal</td>""".r.unanchored
  // Matching: <th scope="row">Energy</th><td>9.6kcal/40kJ</td>
  private final val KJ_KCAL_33_REGEX = """<th scope="row">Energy</th><td>([0-9.]+)kcal/([0-9.]+)kJ</td>""".r.unanchored
  // Matching: <th scope="row">Energy value kJ/(kcal)</th><td>0/(0)</td>
  private final val KJ_KCAL_34_REGEX = """<th scope="row">Energy value kJ/\(kcal\)</th><td>([0-9.]+)/\(([0-9.]+)\)</td>""".r.unanchored
  // Matching: <th scope="row">Energy</th><td>8.4 kJ/ 2.0kcal</td>
  private final val KJ_KCAL_35_REGEX = """<th scope="row">Energy</th><td>([0-9.]+) [a-zA-Z]+/ ([0-9.]+)[a-zA-Z]+</td>""".r.unanchored
