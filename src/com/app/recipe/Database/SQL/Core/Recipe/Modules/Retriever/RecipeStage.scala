 ...
 */
class RecipeStage( database : String ) extends RetrieverCore {

  /**
   * The table name.
   */
  private final val TABLE_NAME = "recipe_stage"

  /**
   * The column names.
   */
  private final val COLUMNS = Array("id","recipe_id","step_id","step_name","description","created_by","created_date","last_updated_by","last_updated_date")
  ...
  override def getRowId( id : Int ) : Option[RecipeStageRow] = {
    getHashMapFromSQL( raw"SELECT * FROM ${database}.${TABLE_NAME} WHERE id = '${id}' ", COLUMNS ) match {
      ...
      case result if result.size == 1 => Some(getObject(result(0), TABLE_NAME).asInstanceOf[RecipeStageRow])
  ...
  def getRecipeId( id : Int ) : Option[List[RecipeStageRow]] = 
    getHashMapFromSQL( raw"SELECT * FROM ${database}.${TABLE_NAME} WHERE recipe_id = '${id}'", COLUMNS ) match {
      ...
      case result => {
        ...
        for( row <- result ) {
          optionList = List(getObject(row, TABLE_NAME).asInstanceOf[RecipeStageRow]) ::: optionList
        }
